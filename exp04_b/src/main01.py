...
file_o.write('file_name,AB_area,EF_area\n')
...
for input_image_path in tqdm.tqdm(input_paths):
    ...
    for tem in range(360):
        ...
        if tem >= 210 and tem <= 270: #ab
            cv2.line(image_rgb_predict, (int(inner_x[0][0]), int(inner_x[0][1])), (int(secs[tem].coords[1][0]), int(secs[tem].coords[1][1])), (0, 0, 255), 1)
            ab_area.append(calculate_distance((int(inner_x[0][0]), int(inner_x[0][1])), (int(secs[tem].coords[1][0]), int(secs[tem].coords[1][1]))))
        if tem >= 90 and tem <= 150: #ef
            cv2.line(image_rgb_predict, (int(inner_x[0][0]), int(inner_x[0][1])), (int(secs[tem].coords[1][0]), int(secs[tem].coords[1][1])), (0, 0, 255), 1)
            ef_area.append(calculate_distance((int(inner_x[0][0]), int(inner_x[0][1])), (int(secs[tem].coords[1][0]), int(secs[tem].coords[1][1]))))
    AB_area = np.sum(ab_area)
    EF_area = np.sum(ef_area)
    ...
    file_o.write('%s,%f,%f\n'%(input_image_path,AB_area,EF_area))
